...
def singleclass_score(classifier, test_pfds, test_target, verbose=False):
    ...
    if not test_target.ndim == 1:
        try:
            feature_target = test_target[..., classifier.targetmap[list(classifier.feature.keys())[0]]]
...
class dataloader(object):
    ...
    def __init__(self, filename, classmap=None):
        ...
        if filename.endswith('.pkl'):
            ...
            try:
                ...
                self.pfds = [ pfdreader(f) for f in data['fname'].astype(str)]
                self.target = np.vstack((data['Overall'], data['Profile'], data['DMCurve'], data['Interval'], data['Subband'])).T
            except IndexError as e:
                data = np.loadtxt(filename, dtype=[('fname', '|S200'), ('Overall', int)], comments='#')
                self.pfds = [ pfdreader(f) for f in data['fname'].astype(str)]
    ...
    def learning_curve(self, classifier,
        ...
        m = y.shape[0]
        stepsize = max(m/25,1)
        ntrials = list(range(1,m,stepsize))
        mm = len(ntrials)
        t_F1 = np.zeros(mm)
        v_F1 = np.zeros(mm)
    ...
    def feature_curve(self, classifier, 
        ...
        if feature == None:
            feature = list(classifier.feature.keys())[0]
