...
#            self.AIonAI = combinedAI([lr,nn,svc, dtree], strategy='lr')
    ...
    def predict(self, pfds, pred_mat=False ):
        ...
        if (self.strategy in self.AIonAIs) and self.strategy not in self.req_predict:
            ...
            if InteractivePy or (len(pfds) < 5*num_workers):
                ...
                threadit.__defaults__[0]['state'] = True
                list_of_predicts = threadpredict_proba(self.list_of_AIs, pfds)
                threadit.__defaults__[0]['state'] = False
        else:
            if InteractivePy or (len(pfds) < 5*num_workers):
        ...
        if self.strategy == 'vote':
            ...
            npc = list(range(self.nclasses))[2:]
            npc.insert(0,0)
    ...
    def report_score(self, pfds, dist='PALFA_Priordists.pkl'):
        ...
        if 'prior_freq_dist' not in self.__dict__:
            import pickle
            import ubc_AI
            ubcAI_path = ubc_AI.__path__[0]
            # Note: we expect a dictionary whose key is 'Pfr_over_Pfp'
            self.prior_freq_dist = pickle.load(open(ubcAI_path + '/' + dist, 'rb'), encoding='latin1')
...
class classifier(object):
    ...
    def __init__(self,feature, use_pca=False, n_comp=12, **kwds):
        if feature == None:
            print("feature == None")
            raise MyError(None)
    ...
    def fit(self, pfds, target, randomshift=False):
        ...
        try:
            if target.ndim == 1:
                ...
                mytarget = target[...,classifier.targetmap[list(self.feature.keys())[0]]]
                
            if self.use_pca:
                self.pca = PCA(n_components=self.n_components, svd_solver='randomized').fit(data[mytarget == 1])
...
def extractfeatures(AIlist, pfds):
    ...
    items = []
    for clf in AIlist:
        items.extend(list(clf.feature.items()))
